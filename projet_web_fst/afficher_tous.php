...
<body>
  ...
  <main>
    ...
    <div class="container">
      ...
      <div class="afficher_tous">
        
        <?php
        $query = "SELECT * FROM  news";
        $ligne = $conn->query($query);
        $Count = $ligne->rowCount();
        //var_dump($Count);
        if( $Count <= 10){
        ?>

        <div class="deuxcolone">

          <?php 
            $contenunews = $conn->query('SELECT * FROM news ORDER BY date_publication DESC');
        ...
        <div class="deuxcolone">
          ...
          <?php 
            $contenunews = $conn->query('SELECT * FROM news ORDER BY date_publication DESC LIMIT 10');
            while ($ligne = $contenunews->fetch()) {
              echo '<div >';
              echo '<a  href="afficher_news.php?id_news='. $ligne['id_news'] .' " class="news1">Date: <span>' . $ligne['date_publication'] . '</span> <br>' ;
              echo 'Titre:<br>' . $ligne['titre'] . '</a>' ;
        ...
        <div class="reste"><a href="afficher_touss.php">.......</a></div>
        
        <?php } ?>

      </div>
      <a href="accueil.php" class="btn_fermer1">&times;</a>
